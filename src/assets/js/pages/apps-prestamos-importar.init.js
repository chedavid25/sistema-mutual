        ...
        for (const row of data) {
            try {
                ...
                const loanData = {
                    ...
                    refinancedAmount: parseFloat(row["Monto Refinanciado"] || row["Refinanciacion"] || row.refinancedAmount || 0),
                    disbursedAmount: parseFloat(row["Monto Prestamo"] || row["Monto Prestamos"] || row["Monto Préstamo"] || row["Monto Préstamos"] || row["Capital"] || row["Importe"] || row["Monto"] || row["Valor"] || row["Capital Original"] || row["Monto Solicitado"] || row.disbursedAmount || 0),
                    expectedAmount,
                    paidAmount,
                    remainingBalance,
                    status,
